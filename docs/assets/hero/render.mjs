...
// A container that pre-installs Chromium usually pins a revision this Playwright
// build does not look for. Point HERO_CHROMIUM at the binary to skip the ladder.
const pinned = process.env.HERO_CHROMIUM;

// Chromium ignores the shell's proxy vars, so behind one the Tailwind CDN and
// Google Fonts fetches fail and waitPainted times out on the JIT gate instead.
const proxyUrl = process.env.HTTPS_PROXY || process.env.https_proxy;
const proxy = proxyUrl ? { server: proxyUrl, bypass: '127.0.0.1,localhost' } : undefined;
...
try {
  browser = await pw.chromium.launch({
    args: ARGS,
    ...(proxy ? { proxy } : {}),
    ...(pinned ? { executablePath: pinned } : useChrome ? { channel: 'chrome' } : {}),
  });
} catch (e) {
  if (useChrome || pinned) throw e;
  console.warn('bundled Chromium unavailable, falling back to system Chrome...');
  browser = await pw.chromium.launch({ args: ARGS, ...(proxy ? { proxy } : {}), channel: 'chrome' });
}
